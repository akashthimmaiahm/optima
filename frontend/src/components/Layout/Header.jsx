...
function NotificationsPanel({ onClose }) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-[#1a1a1f] border border-gray-200 dark:border-[#2a2a35] rounded-xl shadow-2xl z-50 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-[#2a2a35]">
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div key={log.id} className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 dark:border-[#2a2a35] last:border-0 hover:bg-gray-50 dark:hover:bg-[#22222e] transition-colors">
            <div className="w-7 h-7 bg-blue-100 dark:bg-blue-600/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400">{log.user_name?.[0] || '?'}</span>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-600 mt-0.5">{new Date(log.created_at).toLocaleString()}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="px-4 py-2 border-t border-gray-200 dark:border-[#2a2a35]">
        <p className="text-xs text-gray-500 dark:text-gray-600 text-center">Showing last {items.length} events</p>
      </div>
    </div>
  )
}
