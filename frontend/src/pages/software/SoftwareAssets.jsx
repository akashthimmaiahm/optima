...
const licenseTypeColors = { subscription: 'info', perpetual: 'purple', trial: 'warning', usage: 'default', freeware: 'success' }
...
export default function SoftwareAssets() {
  ...
  const load = () => {
    setLoading(true)
    api.get('/software', { params: { search } }).then(r => setSoftware(r.data.data)).finally(() => setLoading(false))
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">{software.length} software applications tracked</p>
        </div>
      ...
      <div className="card p-4 flex gap-3 flex-wrap">
        ...
        <select className="input w-40" value={filterCategory} onChange={e => setFilterCategory(e.target.value)}>
          ...
          {['Productivity', 'Design', 'Development', 'Security', 'Communication', 'CRM', 'Cloud', 'OS', 'Engineering', 'Finance', 'Project Management', 'AI Platform', 'Other'].map(c => <option key={c}>{c}</option>)}
        </select>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="text-center py-12 text-gray-400">No software found</td></tr>
