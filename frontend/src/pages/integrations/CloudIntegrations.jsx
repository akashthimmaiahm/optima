...
const APP_CATALOG = [
  ...
  // ── AI Applications ──────────────────────────────────────────────────────────
  {
    key: 'claude', name: 'Claude (Anthropic)', provider: 'Anthropic', type: 'ai',
    emoji: '🤖', color: 'bg-orange-50 dark:bg-orange-900/20',
    description: 'Track Claude API usage, team seats, and costs. Supports Claude Pro, Team, Enterprise, and API.',
    auth_type: 'api_key', api_endpoint: 'https://api.anthropic.com/v1',
    setup: 'console.anthropic.com → API Keys → Create Key; for Teams: Admin → Members → Manage seats',
    help: [
      'Go to console.anthropic.com and sign in.',
      'Navigate to API Keys → Create Key.',
      'Name it "Optima ITAM" and copy the key (starts with sk-ant-).',
      'For team/organization tracking, go to Settings → Organization.',
      'Copy the Organization ID from the settings page.',
      'The API key provides access to usage data, billing, and model access.',
      'For Claude Team/Enterprise, the admin can view seat counts and spend.',
      'Pricing: Opus $15/$75, Sonnet $3/$15, Haiku $0.25/$1.25 per 1M tokens (input/output).',
    ],
    fields: [
      { key: 'api_key',   label: 'API Key',         placeholder: 'sk-ant-xxxxxxxxxxxxxxxxxxxxx', type: 'password', required: true },
      { key: 'tenant_id', label: 'Organization ID',  placeholder: 'org-xxxxxxxxxxxxxxxx (optional)' },
    ],
  },
  {
    key: 'openai', name: 'ChatGPT / OpenAI', provider: 'OpenAI', type: 'ai',
    emoji: '🧠', color: 'bg-green-50 dark:bg-green-900/20',
    description: 'Monitor OpenAI API usage, ChatGPT Team seats, token consumption, and cost tracking.',
    auth_type: 'api_key', api_endpoint: 'https://api.openai.com/v1',
    setup: 'platform.openai.com → API Keys → Create new secret key; Organization → Settings for Org ID',
    help: [
      'Go to platform.openai.com and sign in.',
      'Navigate to API Keys → Create new secret key.',
      'Name it "Optima ITAM" and copy the key (starts with sk-).',
      'Go to Settings �� Organization → Copy the Organization ID.',
      'For ChatGPT Team/Enterprise, the admin portal shows active seats.',
      'The API key provides read access to usage, billing, and model information.',
      'Pricing: GPT-4o $2.50/$10, GPT-4o-mini $0.15/$0.60 per 1M tokens.',
    ],
    fields: [
      { key: 'api_key',   label: 'API Key',         placeholder: 'sk-xxxxxxxxxxxxxxxxxxxxx', type: 'password', required: true },
      { key: 'tenant_id', label: 'Organization ID',  placeholder: 'org-xxxxxxxxxxxxxxxx (optional)' },
    ],
  },
  {
    key: 'github_copilot', name: 'GitHub Copilot', provider: 'GitHub', type: 'ai',
    emoji: '✈️', color: 'bg-gray-50 dark:bg-gray-800/50',
    description: 'Track GitHub Copilot seats, usage metrics, and active developer count across your organization.',
    auth_type: 'api_key', api_endpoint: 'https://api.github.com',
    setup: 'GitHub → Settings → Developer settings → PAT (Classic) → read:org, copilot scopes',
    help: [
      'Go to github.com → Your profile → Settings → Developer settings.',
      'Click Personal access tokens ��� Tokens (classic) → Generate new token.',
      'Name it "Optima Copilot Tracker" and select scopes: read:org, copilot.',
      'Click "Generate token" and copy it immediately.',
      'Enter your GitHub organization name (the URL slug).',
      'Copilot usage data is available via the /orgs/{org}/copilot/usage API.',
      'Pricing: Individual $10/mo, Business $19/mo, Enterprise $39/mo per seat.',
    ],
    fields: [
      { key: 'api_key',   label: 'Personal Access Token', placeholder: 'ghp_xxxxxxxxxxxxxxxxxxxx', type: 'password', required: true },
      { key: 'tenant_id', label: 'Organization Name',     placeholder: 'your-org-name', required: true },
  ...
  {
    key: 'google_gemini', name: 'Google Gemini', provider: 'Google', type: 'ai',
    emoji: '💎', color: 'bg-blue-50 dark:bg-blue-900/20',
    description: 'Track Gemini API usage, Workspace AI add-on seats, and token consumption costs.',
    auth_type: 'api_key', api_endpoint: 'https://generativelanguage.googleapis.com/v1',
    setup: 'aistudio.google.com → API Key → Create key; or Google Cloud Console → Vertex AI API',
    help: [
      'Go to aistudio.google.com and sign in.',
      'Click "Get API key" → Create API key in new or existing project.',
      'Copy the generated API key.',
      'For Vertex AI (enterprise), use Google Cloud Console → APIs & Services → Credentials.',
      'Enable the Vertex AI API and Generative Language API.',
      'Create a service account key for server-to-server authentication.',
      'Pricing: Gemini Pro $1.25/$5, Flash $0.075/$0.30 per 1M tokens.',
    ],
    fields: [
      { key: 'api_key',   label: 'API Key',     placeholder: 'AIzaSy... (Google AI Studio key)', type: 'password', required: true },
      { key: 'tenant_id', label: 'Project ID',  placeholder: 'GCP Project ID (optional for Vertex AI)' },
    ],
  },
  {
    key: 'microsoft_copilot', name: 'Microsoft Copilot', provider: 'Microsoft', type: 'ai',
    emoji: '🔷', color: 'bg-blue-50 dark:bg-blue-900/20',
    description: 'Track Microsoft 365 Copilot license assignments, usage, and per-seat costs.',
    auth_type: 'oauth2', api_endpoint: 'https://graph.microsoft.com/v1.0',
    setup: 'Same as Microsoft 365 setup — Copilot licenses appear in subscribedSkus',
    help: [
      'Microsoft Copilot licensing is managed through Microsoft 365 admin.',
      'If you already have Microsoft 365 connected, Copilot SKUs will appear automatically.',
      'Otherwise, follow the same setup as Microsoft 365 integration.',
      'Go to portal.azure.com → App registrations → Create or reuse existing app.',
      'Ensure the app has User.Read.All and Organization.Read.All permissions.',
      'Copilot seats are tracked via subscribedSkus API with specific SKU part numbers.',
      'Pricing: M365 Copilot $30/user/month.',
    ],
    fields: [
      { key: 'client_id',     label: 'Application (Client) ID', placeholder: 'Azure AD app client ID', required: true },
      { key: 'client_secret', label: 'Client Secret',           placeholder: 'App registration secret', type: 'password', required: true },
      { key: 'tenant_id',     label: 'Tenant ID',               placeholder: 'Azure AD tenant GUID', required: true },
    ],
  },
  {
    key: 'cursor', name: 'Cursor', provider: 'Anysphere', type: 'ai',
    emoji: '⚡', color: 'bg-cyan-50 dark:bg-cyan-900/20',
    description: 'Track Cursor AI code editor seats and team usage for development teams.',
    auth_type: 'api_key', api_endpoint: 'https://api.cursor.com',
    setup: 'cursor.com → Settings → Team → Billing for seat counts; API access via team admin',
    help: [
      'Sign in to cursor.com with your team admin account.',
      'Go to Settings → Team → Members to view active seats.',
      'Cursor does not currently expose a public API for seat management.',
      'Use this integration to manually track Cursor licenses and costs.',
      'Enter the number of seats and plan type for cost tracking.',
      'Pricing: Hobby (Free), Pro $20/mo, Business $40/mo per seat.',
    ],
    fields: [
      { key: 'api_key',   label: 'Team Admin Email',  placeholder: 'admin@yourcompany.com', required: true },
      { key: 'tenant_id', label: 'Team Name',         placeholder: 'Your Cursor team name' },
    ],
  },
  {
    key: 'midjourney', name: 'Midjourney', provider: 'Midjourney', type: 'ai',
    emoji: '🎨', color: 'bg-pink-50 dark:bg-pink-900/20',
    description: 'Track Midjourney subscriptions and image generation usage across your organization.',
    auth_type: 'api_key', api_endpoint: 'https://midjourney.com',
    setup: 'midjourney.com → Manage Subscription → Track plan tier and member count',
    help: [
      'Midjourney does not currently provide a public API.',
      'Use this integration to manually track subscription plans and costs.',
      'Sign in to midjourney.com and go to Manage Subscription.',
      'Note your plan tier: Basic ($10), Standard ($30), Pro ($60), Mega ($120).',
      'Track the number of team members with active subscriptions.',
      'Enter an admin email for contact and the team/server name.',
    ],
    fields: [
      { key: 'api_key',   label: 'Admin Email',       placeholder: 'admin@yourcompany.com', required: true },
      { key: 'tenant_id', label: 'Discord Server ID',  placeholder: 'Your Midjourney Discord server (optional)' },
    ],
  },
  {
    key: 'perplexity', name: 'Perplexity AI', provider: 'Perplexity', type: 'ai',
    emoji: '🔍', color: 'bg-teal-50 dark:bg-teal-900/20',
    description: 'Track Perplexity Pro/Enterprise seats and API usage for research teams.',
    auth_type: 'api_key', api_endpoint: 'https://api.perplexity.ai',
    setup: 'perplexity.ai → Settings → API → Generate API key; or Enterprise admin for seat management',
    help: [
      'Go to perplexity.ai and sign in.',
      'Navigate to Settings → API → Generate new API key.',
      'Copy the API key for usage tracking.',
      'For Perplexity Enterprise, contact your admin for seat count data.',
      'Pricing: Free, Pro $20/mo, Enterprise custom pricing.',
    ],
    fields: [
      { key: 'api_key',   label: 'API Key',         placeholder: 'pplx-xxxxxxxxxxxxxxxxxxxxx', type: 'password', required: true },
      { key: 'tenant_id', label: 'Organization',     placeholder: 'Organization name (optional)' },
    ],
  },
]
...
const providerMeta = {
  ...
  Anthropic:  { emoji: '🤖', color: 'bg-orange-50 dark:bg-orange-900/20' },
  OpenAI:     { emoji: '🧠', color: 'bg-green-50 dark:bg-green-900/20' },
  GitHub:     { emoji: '✈️', color: 'bg-gray-50 dark:bg-gray-800/50' },
  Anysphere:  { emoji: '⚡', color: 'bg-cyan-50 dark:bg-cyan-900/20' },
  Midjourney: { emoji: '🎨', color: 'bg-pink-50 dark:bg-pink-900/20' },
  Perplexity: { emoji: '🔍', color: 'bg-teal-50 dark:bg-teal-900/20' },
}
...
const typeColors = {
  ...
  security: 'danger', monitoring: 'purple', hr: 'success', ai: 'purple',
}
